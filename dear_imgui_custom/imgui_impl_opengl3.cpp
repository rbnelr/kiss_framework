...
// >>>> MODIFIED:
	...
	#include "opengl.hpp" // for OGL_DBG_LABEL

	#define IMGUI_IMPL_OPENGL_LOADER_CUSTOM

	// Removed all state backup (glGet*) since on my desktop (only in fullscreen)
	//  glGet was causing really high (22% on 12 core cpu) cpu usage (wierd driver mal-optimization?)
	// I never rely on state being kept across render calls anyway
	
	// Disabled glGen and glDelete of VAO each frame just in case the driver pulls another stunt like with the glGet calls
	// Just create a globabl VAO, I'm not sure how to support multiple instances of imgui anyway
// <<<<
...
bool    ImGui_ImplOpenGL3_Init(const char* glsl_version)
{
    ...
    */
	...
	OGL_DBG_LABEL(GL_VERTEX_ARRAY, bd->Vao, "imgui_vao");
...
void    ImGui_ImplOpenGL3_RenderDrawData(ImDrawData* draw_data)
{
    ...
    ImGui_ImplOpenGL3_SetupRenderState(draw_data, fb_width, fb_height, bd->Vao);
...
bool ImGui_ImplOpenGL3_CreateFontsTexture()
{
	...
	OGL_DBG_LABEL(GL_TEXTURE, bd->FontTexture, "imgui_font_tex");
