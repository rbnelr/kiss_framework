...
#ifndef _WIN32
static_assert(false, "implement directory change polling on other platforms");
#endif
			...
			for (;;) { // for all files returned by FindFirstFileW / FindNextFileW
				...
				if (ret == 0) {
					if (err == ERROR_NO_MORE_FILES) {
						...
						assert(false);
						fprintf(stderr, "find_files: FindNextFile failed! [%x]", (unsigned int)err);
