// file was generated by kissmath.py at <TODO: add github link>

////// Inline definitions

#include "int64v2.hpp"
#include "int2.hpp"
#include "float4.hpp"
#include "bool2.hpp"
#include "float3.hpp"
#include "uint8v2.hpp"

namespace kissmath {
	//// forward declarations
	// typedef these because the _t suffix is kinda unwieldy when using these types often
	
	typedef int64_t int64;
	typedef uint8_t uint8;
	
	// Component indexing operator
	inline constexpr float& float2::operator[] (int i) {
		return arr[i];
	}
	
	// Component indexing operator
	inline constexpr float const& float2::operator[] (int i) const {
		return arr[i];
	}
	
	
	// sets all components to one value
	// implicit constructor -> float3(x,y,z) * 5 will be turned into float3(x,y,z) * float3(5) by to compiler to be able to execute operator*(float3, float3), which is desirable
	// and short initialization like float3 a = 0; works
	inline constexpr float2::float2 (float all): x{all}, y{all} {
		
	}
	
	// supply all components
	inline constexpr float2::float2 (float x, float y): x{x}, y{y} {
		
	}
	
	// truncate vector
	inline constexpr float2::float2 (float3 v): x{v.x}, y{v.y} {
		
	}
	
	// truncate vector
	inline constexpr float2::float2 (float4 v): x{v.x}, y{v.y} {
		
	}
	
	//// Truncating cast operators
	
	
	//// Type cast operators
	
	
	// type cast operator
	inline constexpr float2::operator bool2 () const {
		return bool2((bool)x, (bool)y);
	}
	
	// type cast operator
	inline constexpr float2::operator int2 () const {
		return int2((int)x, (int)y);
	}
	
	// type cast operator
	inline constexpr float2::operator int64v2 () const {
		return int64v2((int64)x, (int64)y);
	}
	
	// type cast operator
	inline constexpr float2::operator uint8v2 () const {
		return uint8v2((uint8)x, (uint8)y);
	}
	
	
	// componentwise arithmetic operator
	inline float2 float2::operator+= (float2 r) {
		x += r.x;
		y += r.y;
		return *this;
	}
	
	// componentwise arithmetic operator
	inline float2 float2::operator-= (float2 r) {
		x -= r.x;
		y -= r.y;
		return *this;
	}
	
	// componentwise arithmetic operator
	inline float2 float2::operator*= (float2 r) {
		x *= r.x;
		y *= r.y;
		return *this;
	}
	
	// componentwise arithmetic operator
	inline float2 float2::operator/= (float2 r) {
		x /= r.x;
		y /= r.y;
		return *this;
	}
	
	//// arthmethic ops
	
	
	inline constexpr float2 operator+ (float2 v) {
		return float2(+v.x, +v.y);
	}
	
	inline constexpr float2 operator- (float2 v) {
		return float2(-v.x, -v.y);
	}
	
	inline constexpr float2 operator+ (float2 l, float2 r) {
		return float2(l.x + r.x, l.y + r.y);
	}
	
	inline constexpr float2 operator- (float2 l, float2 r) {
		return float2(l.x - r.x, l.y - r.y);
	}
	
	inline constexpr float2 operator* (float2 l, float2 r) {
		return float2(l.x * r.x, l.y * r.y);
	}
	
	inline constexpr float2 operator/ (float2 l, float2 r) {
		return float2(l.x / r.x, l.y / r.y);
	}
	
	//// bitwise ops
	
	
	//// comparison ops
	
	
	// componentwise comparison returns a bool vector
	inline constexpr bool2 operator< (float2 l, float2 r) {
		return bool2(l.x < r.x, l.y < r.y);
	}
	
	// componentwise comparison returns a bool vector
	inline constexpr bool2 operator<= (float2 l, float2 r) {
		return bool2(l.x <= r.x, l.y <= r.y);
	}
	
	// componentwise comparison returns a bool vector
	inline constexpr bool2 operator> (float2 l, float2 r) {
		return bool2(l.x > r.x, l.y > r.y);
	}
	
	// componentwise comparison returns a bool vector
	inline constexpr bool2 operator>= (float2 l, float2 r) {
		return bool2(l.x >= r.x, l.y >= r.y);
	}
	
	// componentwise equality comparison, returns a bool vector
	inline constexpr bool2 equal (float2 l, float2 r) {
		return bool2(l.x == r.x, l.y == r.y);
	}
	
	// componentwise inequality comparison, returns a bool vector
	inline constexpr bool2 nequal (float2 l, float2 r) {
		return bool2(l.x != r.x, l.y != r.y);
	}
	
	// full equality comparison, returns true only if all components are equal
	inline constexpr bool operator== (float2 l, float2 r) {
		return (l.x == r.x) && (l.y == r.y);
	}
	
	// full inequality comparison, returns true if any components are inequal
	inline constexpr bool operator!= (float2 l, float2 r) {
		return (l.x != r.x) || (l.y != r.y);
	}
	
	// componentwise ternary (c ? l : r)
	inline constexpr float2 select (bool2 c, float2 l, float2 r) {
		return float2(c.x ? l.x : r.x, c.y ? l.y : r.y);
	}
	
	//// misc ops
	
	// componentwise absolute
	inline float2 abs (float2 v) {
		return float2(abs(v.x), abs(v.y));
	}
	
	// componentwise minimum
	inline constexpr float2 min (float2 l, float2 r) {
		return float2(min(l.x,r.x), min(l.y,r.y));
	}
	
	// componentwise maximum
	inline constexpr float2 max (float2 l, float2 r) {
		return float2(max(l.x,r.x), max(l.y,r.y));
	}
	
	// componentwise clamp into range [a,b]
	inline constexpr float2 clamp (float2 x, float2 a, float2 b) {
		return min(max(x,a), b);
	}
	
	// componentwise clamp into range [0,1] also known as saturate in hlsl
	inline constexpr float2 clamp (float2 x) {
		return min(max(x, float(0)), float(1));
	}
	
	// get minimum component of vector, optionally get component index via min_index
	inline float min_component (float2 v, int* min_index) {
		int index = 0;
		float min_val = v.x;	
		for (int i=1; i<2; ++i) {
			if (v.arr[i] <= min_val) {
				index = i;
				min_val = v.arr[i];
			}
		}
		if (min_index) *min_index = index;
		return min_val;
	}
	
	// get maximum component of vector, optionally get component index via max_index
	inline float max_component (float2 v, int* max_index) {
		int index = 0;
		float max_val = v.x;	
		for (int i=1; i<2; ++i) {
			if (v.arr[i] >= max_val) {
				index = i;
				max_val = v.arr[i];
			}
		}
		if (max_index) *max_index = index;
		return max_val;
	}
	
	
	// componentwise floor
	inline float2 floor (float2 v) {
		return float2(floor(v.x), floor(v.y));
	}
	
	// componentwise ceil
	inline float2 ceil (float2 v) {
		return float2(ceil(v.x), ceil(v.y));
	}
	
	// componentwise round
	inline float2 round (float2 v) {
		return float2(round(v.x), round(v.y));
	}
	
	// componentwise floor to int
	inline int2 floori (float2 v) {
		return int2(floori(v.x), floori(v.y));
	}
	
	// componentwise ceil to int
	inline int2 ceili (float2 v) {
		return int2(ceili(v.x), ceili(v.y));
	}
	
	// componentwise round to int
	inline int2 roundi (float2 v) {
		return int2(roundi(v.x), roundi(v.y));
	}
	
	// componentwise pow
	inline float2 pow (float2 v, float2 e) {
		return float2(pow(v.x,e.x), pow(v.y,e.y));
	}
	
	// componentwise wrap
	inline float2 wrap (float2 v, float2 range) {
		return float2(wrap(v.x,range.x), wrap(v.y,range.y));
	}
	
	// componentwise wrap
	inline float2 wrap (float2 v, float2 a, float2 b) {
		return float2(wrap(v.x,a.x,b.x), wrap(v.y,a.y,b.y));
	}
	
	
	//// Angle conversion
	
	
	// converts degrees to radiants
	inline constexpr float2 to_radians (float2 deg) {
		return deg * DEG_TO_RAD;
	}
	
	// converts radiants to degrees
	inline constexpr float2 to_degrees (float2 rad) {
		return rad * RAD_TO_DEG;
	}
	
	// converts degrees to radiants
	// shortform to make degree literals more readable
	inline constexpr float2 deg (float2 deg) {
		return deg * DEG_TO_RAD;
	}
	
	// linear interpolation
	// like getting the output of a linear function
	// ex. t=0 -> a ; t=1 -> b ; t=0.5 -> (a+b)/2
	inline constexpr float2 lerp (float2 a, float2 b, float2 t) {
		return t * (b - a) + a;
	}
	
	// linear mapping
	// sometimes called inverse linear interpolation
	// like getting the x for a y on a linear function
	// ex. map(70, 0,100) -> 0.7 ; map(0.5, -1,+1) -> 0.75
	inline constexpr float2 map (float2 x, float2 in_a, float2 in_b) {
		return (x - in_a) / (in_b - in_a);
	}
	
	// linear remapping
	// equivalent of lerp(out_a, out_b, map(x, in_a, in_b))
	inline constexpr float2 map (float2 x, float2 in_a, float2 in_b, float2 out_a, float2 out_b) {
		return lerp(out_a, out_b, map(x, in_a, in_b));
	}
	
	//// Various interpolation
	
	
	// standard smoothstep interpolation
	inline float2 smoothstep (float2 x) {
		float2 t = clamp(x);
		return t * t * (3.0f - 2.0f * t);
	}
	
	// 3 point bezier interpolation
	inline float2 bezier (float2 a, float2 b, float2 c, float t) {
		float2 d = lerp(a, b, t);
		float2 e = lerp(b, c, t);
		float2 f = lerp(d, e, t);
		return f;
	}
	
	// 4 point bezier interpolation
	inline float2 bezier (float2 a, float2 b, float2 c, float2 d, float t) {
		return bezier(
					  lerp(a, b, t),
					  lerp(b, c, t),
					  lerp(c, d, t),
					  t
			   );
	}
	
	// 5 point bezier interpolation
	inline float2 bezier (float2 a, float2 b, float2 c, float2 d, float2 e, float t) {
		return bezier(
					  lerp(a, b, t),
					  lerp(b, c, t),
					  lerp(c, d, t),
					  lerp(d, e, t),
					  t
			   );
	}
	
	//// Vector math
	
	
	// magnitude of vector
	inline float length (float2 v) {
		return sqrt((float)(v.x * v.x + v.y * v.y));
	}
	
	// squared magnitude of vector, cheaper than length() because it avoids the sqrt(), some algorithms only need the squared magnitude
	inline constexpr float length_sqr (float2 v) {
		return v.x * v.x + v.y * v.y;
	}
	
	// distance between points, equivalent to length(a - b)
	inline float distance (float2 a, float2 b) {
		return length(a - b);
	}
	
	// normalize vector so that it has length() = 1, undefined for zero vector
	inline float2 normalize (float2 v) {
		return float2(v) / length(v);
	}
	
	// normalize vector so that it has length() = 1, returns zero vector if vector was zero vector
	inline float2 normalizesafe (float2 v) {
		float len = length(v);
		if (len == float(0)) {
			return float(0);
		}
		return float2(v) / float2(len);
	}
	
	// dot product
	inline constexpr float dot (float2 l, float2 r) {
		return l.x * r.x + l.y * r.y;
	}
	
	// 2d cross product hack for convenient 2d stuff
	// same as cross({T.name[:-2]}3(l, 0), {T.name[:-2]}3(r, 0)).z,
	// ie. the cross product of the 2d vectors on the z=0 plane in 3d space and then return the z coord of that (signed mag of cross product)
	inline constexpr float cross (float2 l, float2 r) {
		return l.x * r.y - l.y * r.x;
	}
	
	// rotate 2d vector counterclockwise 90 deg, ie. float2(-y, x) which is fast
	inline constexpr float2 rotate90 (float2 v) {
		return float2(-v.y, v.x);
	}
}

