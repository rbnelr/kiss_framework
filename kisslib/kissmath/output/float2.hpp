...
namespace kissmath {
	...
	struct float2 {
		...
		inline constexpr float2 (float all);
		
		// supply all components
		inline constexpr float2 (float x, float y);
		
		// truncate vector
		inline constexpr float2 (float3 v);
		
		// truncate vector
		inline constexpr float2 (float4 v);
	...
	inline float min_component (float2 v, int* min_index=nullptr);
	
	// get maximum component of vector, optionally get component index via max_index
	inline float max_component (float2 v, int* max_index=nullptr);
