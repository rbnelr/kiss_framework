...
namespace kissmath {
	...
	inline constexpr float& float3::operator[] (int i) {
		return arr[i];
	}
	
	// Component indexing operator
	inline constexpr float const& float3::operator[] (int i) const {
		return arr[i];
	}
	...
	inline constexpr float3::float3 (float all): x{all}, y{all}, z{all} {
	...
	// extend vector
	inline constexpr float3::float3 (float2 xy, float z): x{xy.x}, y{xy.y}, z{z} {
		
	}
	
	// truncate vector
	inline constexpr float3::float3 (float4 v): x{v.x}, y{v.y}, z{v.z} {
	...
	inline constexpr float3::operator float2 () const {
		return float2(x, y);
	}
	...
	// type cast operator
	inline constexpr float3::operator bool3 () const {
		return bool3((bool)x, (bool)y, (bool)z);
	}
	
	// type cast operator
	inline constexpr float3::operator int3 () const {
		return int3((int)x, (int)y, (int)z);
	}
	
	// type cast operator
	inline constexpr float3::operator int64v3 () const {
		return int64v3((int64)x, (int64)y, (int64)z);
	}
	
	// type cast operator
	inline constexpr float3::operator uint8v3 () const {
		return uint8v3((uint8)x, (uint8)y, (uint8)z);
	}
	
	
	// componentwise arithmetic operator
	inline float3 float3::operator+= (float3 r) {
		x += r.x;
		y += r.y;
		z += r.z;
		return *this;
	...
	inline constexpr bool3 equal (float3 l, float3 r) {
		return bool3(l.x == r.x, l.y == r.y, l.z == r.z);
	}
	
	// componentwise inequality comparison, returns a bool vector
	inline constexpr bool3 nequal (float3 l, float3 r) {
		return bool3(l.x != r.x, l.y != r.y, l.z != r.z);
	}
	
	// full equality comparison, returns true only if all components are equal
	inline constexpr bool operator== (float3 l, float3 r) {
	...
	inline float min_component (float3 v, int* min_index) {
		int index = 0;
		float min_val = v.x;	
		for (int i=1; i<3; ++i) {
			if (v.arr[i] <= min_val) {
	...
	inline float max_component (float3 v, int* max_index) {
		int index = 0;
		float max_val = v.x;	
		for (int i=1; i<3; ++i) {
			if (v.arr[i] >= max_val) {
