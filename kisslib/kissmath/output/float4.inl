...
namespace kissmath {
	...
	inline constexpr float& float4::operator[] (int i) {
		return arr[i];
	}
	
	// Component indexing operator
	inline constexpr float const& float4::operator[] (int i) const {
		return arr[i];
	}
	...
	inline constexpr float4::float4 (float all): x{all}, y{all}, z{all}, w{all} {
	...
	// extend vector
	inline constexpr float4::float4 (float2 xy, float z, float w): x{xy.x}, y{xy.y}, z{z}, w{w} {
		
	}
	
	// extend vector
	inline constexpr float4::float4 (float3 xyz, float w): x{xyz.x}, y{xyz.y}, z{xyz.z}, w{w} {
	...
	inline constexpr float4::operator float2 () const {
		return float2(x, y);
	}
	
	// truncating cast operator
	inline constexpr float4::operator float3 () const {
		return float3(x, y, z);
	}
	...
	// type cast operator
	inline constexpr float4::operator bool4 () const {
		return bool4((bool)x, (bool)y, (bool)z, (bool)w);
	}
	
	// type cast operator
	inline constexpr float4::operator int4 () const {
		return int4((int)x, (int)y, (int)z, (int)w);
	}
	
	// type cast operator
	inline constexpr float4::operator int64v4 () const {
		return int64v4((int64)x, (int64)y, (int64)z, (int64)w);
	}
	
	// type cast operator
	inline constexpr float4::operator uint8v4 () const {
		return uint8v4((uint8)x, (uint8)y, (uint8)z, (uint8)w);
	}
	
	
	// componentwise arithmetic operator
	inline float4 float4::operator+= (float4 r) {
		x += r.x;
		y += r.y;
		z += r.z;
		w += r.w;
	...
	inline float min_component (float4 v, int* min_index) {
		int index = 0;
		float min_val = v.x;	
		for (int i=1; i<4; ++i) {
			if (v.arr[i] <= min_val) {
	...
	inline float max_component (float4 v, int* max_index) {
		int index = 0;
		float max_val = v.x;	
		for (int i=1; i<4; ++i) {
			if (v.arr[i] >= max_val) {
