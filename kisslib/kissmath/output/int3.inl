// file was generated by kissmath.py at <TODO: add github link>

////// Inline definitions

#include "int2.hpp"
#include "bool3.hpp"
#include "int4.hpp"
#include "uint8v3.hpp"
#include "float3.hpp"
#include "int64v3.hpp"

namespace kissmath {
	//// forward declarations
	// typedef these because the _t suffix is kinda unwieldy when using these types often
	
	typedef uint8_t uint8;
	typedef int64_t int64;
	
	// Component indexing operator
	inline constexpr int& int3::operator[] (int i) {
		return arr[i];
	}
	
	// Component indexing operator
	inline constexpr int const& int3::operator[] (int i) const {
		return arr[i];
	}
	
	
	// sets all components to one value
	// implicit constructor -> float3(x,y,z) * 5 will be turned into float3(x,y,z) * float3(5) by to compiler to be able to execute operator*(float3, float3), which is desirable
	// and short initialization like float3 a = 0; works
	inline constexpr int3::int3 (int all): x{all}, y{all}, z{all} {
		
	}
	
	// supply all components
	inline constexpr int3::int3 (int x, int y, int z): x{x}, y{y}, z{z} {
		
	}
	
	// extend vector
	inline constexpr int3::int3 (int2 xy, int z): x{xy.x}, y{xy.y}, z{z} {
		
	}
	
	// truncate vector
	inline constexpr int3::int3 (int4 v): x{v.x}, y{v.y}, z{v.z} {
		
	}
	
	//// Truncating cast operators
	
	
	// truncating cast operator
	inline constexpr int3::operator int2 () const {
		return int2(x, y);
	}
	
	//// Type cast operators
	
	
	// type cast operator
	inline constexpr int3::operator bool3 () const {
		return bool3((bool)x, (bool)y, (bool)z);
	}
	
	// type cast operator
	inline constexpr int3::operator float3 () const {
		return float3((float)x, (float)y, (float)z);
	}
	
	// type cast operator
	inline constexpr int3::operator int64v3 () const {
		return int64v3((int64)x, (int64)y, (int64)z);
	}
	
	// type cast operator
	inline constexpr int3::operator uint8v3 () const {
		return uint8v3((uint8)x, (uint8)y, (uint8)z);
	}
	
	
	// componentwise arithmetic operator
	inline int3 int3::operator+= (int3 r) {
		x += r.x;
		y += r.y;
		z += r.z;
		return *this;
	}
	
	// componentwise arithmetic operator
	inline int3 int3::operator-= (int3 r) {
		x -= r.x;
		y -= r.y;
		z -= r.z;
		return *this;
	}
	
	// componentwise arithmetic operator
	inline int3 int3::operator*= (int3 r) {
		x *= r.x;
		y *= r.y;
		z *= r.z;
		return *this;
	}
	
	// componentwise arithmetic operator
	inline int3 int3::operator/= (int3 r) {
		x /= r.x;
		y /= r.y;
		z /= r.z;
		return *this;
	}
	
	//// arthmethic ops
	
	
	inline constexpr int3 operator+ (int3 v) {
		return int3(+v.x, +v.y, +v.z);
	}
	
	inline constexpr int3 operator- (int3 v) {
		return int3(-v.x, -v.y, -v.z);
	}
	
	inline constexpr int3 operator+ (int3 l, int3 r) {
		return int3(l.x + r.x, l.y + r.y, l.z + r.z);
	}
	
	inline constexpr int3 operator- (int3 l, int3 r) {
		return int3(l.x - r.x, l.y - r.y, l.z - r.z);
	}
	
	inline constexpr int3 operator* (int3 l, int3 r) {
		return int3(l.x * r.x, l.y * r.y, l.z * r.z);
	}
	
	inline constexpr int3 operator/ (int3 l, int3 r) {
		return int3(l.x / r.x, l.y / r.y, l.z / r.z);
	}
	
	//// bitwise ops
	
	
	inline constexpr int3 operator~ (int3 v) {
		return int3(~v.x, ~v.y, ~v.z);
	}
	
	inline constexpr int3 operator& (int3 l, int3 r) {
		return int3(l.x & r.x, l.y & r.y, l.z & r.z);
	}
	
	inline constexpr int3 operator| (int3 l, int3 r) {
		return int3(l.x | r.x, l.y | r.y, l.z | r.z);
	}
	
	inline constexpr int3 operator^ (int3 l, int3 r) {
		return int3(l.x ^ r.x, l.y ^ r.y, l.z ^ r.z);
	}
	
	inline constexpr int3 operator<< (int3 l, int3 r) {
		return int3(l.x << r.x, l.y << r.y, l.z << r.z);
	}
	
	inline constexpr int3 operator>> (int3 l, int3 r) {
		return int3(l.x >> r.x, l.y >> r.y, l.z >> r.z);
	}
	
	//// comparison ops
	
	
	// componentwise comparison returns a bool vector
	inline constexpr bool3 operator< (int3 l, int3 r) {
		return bool3(l.x < r.x, l.y < r.y, l.z < r.z);
	}
	
	// componentwise comparison returns a bool vector
	inline constexpr bool3 operator<= (int3 l, int3 r) {
		return bool3(l.x <= r.x, l.y <= r.y, l.z <= r.z);
	}
	
	// componentwise comparison returns a bool vector
	inline constexpr bool3 operator> (int3 l, int3 r) {
		return bool3(l.x > r.x, l.y > r.y, l.z > r.z);
	}
	
	// componentwise comparison returns a bool vector
	inline constexpr bool3 operator>= (int3 l, int3 r) {
		return bool3(l.x >= r.x, l.y >= r.y, l.z >= r.z);
	}
	
	// componentwise equality comparison, returns a bool vector
	inline constexpr bool3 equal (int3 l, int3 r) {
		return bool3(l.x == r.x, l.y == r.y, l.z == r.z);
	}
	
	// componentwise inequality comparison, returns a bool vector
	inline constexpr bool3 nequal (int3 l, int3 r) {
		return bool3(l.x != r.x, l.y != r.y, l.z != r.z);
	}
	
	// full equality comparison, returns true only if all components are equal
	inline constexpr bool operator== (int3 l, int3 r) {
		return (l.x == r.x) && (l.y == r.y) && (l.z == r.z);
	}
	
	// full inequality comparison, returns true if any components are inequal
	inline constexpr bool operator!= (int3 l, int3 r) {
		return (l.x != r.x) || (l.y != r.y) || (l.z != r.z);
	}
	
	// componentwise ternary (c ? l : r)
	inline constexpr int3 select (bool3 c, int3 l, int3 r) {
		return int3(c.x ? l.x : r.x, c.y ? l.y : r.y, c.z ? l.z : r.z);
	}
	
	//// misc ops
	
	// componentwise absolute
	inline int3 abs (int3 v) {
		return int3(abs(v.x), abs(v.y), abs(v.z));
	}
	
	// componentwise minimum
	inline constexpr int3 min (int3 l, int3 r) {
		return int3(min(l.x,r.x), min(l.y,r.y), min(l.z,r.z));
	}
	
	// componentwise maximum
	inline constexpr int3 max (int3 l, int3 r) {
		return int3(max(l.x,r.x), max(l.y,r.y), max(l.z,r.z));
	}
	
	// componentwise clamp into range [a,b]
	inline constexpr int3 clamp (int3 x, int3 a, int3 b) {
		return min(max(x,a), b);
	}
	
	// componentwise clamp into range [0,1] also known as saturate in hlsl
	inline constexpr int3 clamp (int3 x) {
		return min(max(x, int(0)), int(1));
	}
	
	// get minimum component of vector, optionally get component index via min_index
	inline int min_component (int3 v, int* min_index) {
		int index = 0;
		int min_val = v.x;	
		for (int i=1; i<3; ++i) {
			if (v.arr[i] <= min_val) {
				index = i;
				min_val = v.arr[i];
			}
		}
		if (min_index) *min_index = index;
		return min_val;
	}
	
	// get maximum component of vector, optionally get component index via max_index
	inline int max_component (int3 v, int* max_index) {
		int index = 0;
		int max_val = v.x;	
		for (int i=1; i<3; ++i) {
			if (v.arr[i] >= max_val) {
				index = i;
				max_val = v.arr[i];
			}
		}
		if (max_index) *max_index = index;
		return max_val;
	}
	
	
	// componentwise wrap
	inline int3 wrap (int3 v, int3 range) {
		return int3(wrap(v.x,range.x), wrap(v.y,range.y), wrap(v.z,range.z));
	}
	
	// componentwise wrap
	inline int3 wrap (int3 v, int3 a, int3 b) {
		return int3(wrap(v.x,a.x,b.x), wrap(v.y,a.y,b.y), wrap(v.z,a.z,b.z));
	}
	
	
	//// Vector math
	
	
	// magnitude of vector
	inline float length (int3 v) {
		return sqrt((float)(v.x * v.x + v.y * v.y + v.z * v.z));
	}
	
	// squared magnitude of vector, cheaper than length() because it avoids the sqrt(), some algorithms only need the squared magnitude
	inline constexpr int length_sqr (int3 v) {
		return v.x * v.x + v.y * v.y + v.z * v.z;
	}
	
	// distance between points, equivalent to length(a - b)
	inline float distance (int3 a, int3 b) {
		return length(a - b);
	}
	
	// normalize vector so that it has length() = 1, undefined for zero vector
	inline float3 normalize (int3 v) {
		return float3(v) / length(v);
	}
	
	// normalize vector so that it has length() = 1, returns zero vector if vector was zero vector
	inline float3 normalizesafe (int3 v) {
		float len = length(v);
		if (len == float(0)) {
			return float(0);
		}
		return float3(v) / float3(len);
	}
	
	// dot product
	inline constexpr int dot (int3 l, int3 r) {
		return l.x * r.x + l.y * r.y + l.z * r.z;
	}
	
	// 3d cross product
	inline constexpr int3 cross (int3 l, int3 r) {
		return int3(
					l.y * r.z - l.z * r.y,
					l.z * r.x - l.x * r.z,
					l.x * r.y - l.y * r.x);
	}
}

